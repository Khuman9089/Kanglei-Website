...
export async function POST(req: Request) {
  try {
    ...
    // Check if action is sending a notification
    if (body.action === 'send_notification') {
      const { title, body: msgBody, target_screen, target_id } = body;
      if (!title || !msgBody) {
      ...
      const newNotif = {
        ...
        target_screen: target_screen || 'useful',
        target_id: target_id || '',
        sent: true,
      };

      current.notifications = [newNotif, ...(current.notifications || [])];
      writeSettings(current);

      return NextResponse.json({ success: true, notification: newNotif, message: 'Push notification broadcasted successfully' });
    }

    // Otherwise update features / ads / side_menu
    const updated = {
      ...current,
      features: body.features ? { ...current.features, ...body.features } : current.features,
      ads: body.ads ? { ...current.ads, ...body.ads } : current.ads,
      side_menu: body.side_menu ? { ...current.side_menu, ...body.side_menu } : (current.side_menu || DEFAULT_SETTINGS.side_menu),
      notifications: body.notifications || current.notifications,
    };

    writeSettings(updated);

    return NextResponse.json({ success: true, settings: updated, message: 'App Control settings updated successfully' });
  } catch (err: any) {
