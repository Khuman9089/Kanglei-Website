...
import { X, Sparkles, ArrowRight, Sun, Moon } from 'lucide-react';
...
export default function BannerAd970x90() {
  ...
  // Do NOT show banner ad or mobile buttons on admin pages, astrologer dashboard, shop pages, app screen, calendar, or astroai
  if (pathname?.startsWith('/admin') || pathname?.startsWith('/dashboard/astrologer') || pathname?.startsWith('/shop') || pathname?.startsWith('/app') || pathname?.startsWith('/calendar') || pathname?.startsWith('/astroai')) {
    return null;
  }
  ...
  return (
    <aside aria-label="Advertisement & Quick Services Bar" className="w-full bg-[#fffdfa] py-3 px-4 print:hidden flex flex-col justify-center items-center relative z-20">
        ...
        <div className={`w-full max-w-[970px] min-h-[90px] rounded-2xl bg-gradient-to-r ${themeClasses} border-2 shadow-xl p-3.5 flex flex-wrap sm:flex-nowrap items-center justify-between gap-4 relative overflow-hidden`}>
          ...
          <div className="flex items-center gap-3 shrink-0 z-10">
            ...
            <button
              ...
              className="p-1.5 rounded-lg bg-black/30 text-slate-300 hover:text-white hover:bg-black/60 transition-colors cursor-pointer"
              title="Dismiss Ad"
            >
              <X className="w-4 h-4" />
            </button>
      ...
      {/* 2. Mobile Quick Action Buttons Below Banner Ad (Visible on Mobile View) */}
      <div className="w-full max-w-[970px] mt-2.5 px-0.5 md:hidden flex items-center justify-between gap-2 text-xs font-bold z-10">
        <Link
          href="/numit_leppa_yengba"
          className="flex-1 py-2.5 px-2 rounded-full bg-gradient-to-r from-[#e65c00] to-[#f97316] text-white font-extrabold text-[11px] sm:text-xs text-center shadow-md flex items-center justify-center gap-1.5 hover:opacity-95 transition-all truncate"
        >
          <Sun className="w-3.5 h-3.5 text-yellow-200 fill-yellow-200 shrink-0" />
          <span className="truncate">Numit Leppa</span>
        </Link>

        <Link
          href="/manipuri_kuthi_yengba"
          className="flex-1 py-2.5 px-2 rounded-full bg-gradient-to-r from-[#d97706] to-[#f59e0b] text-white font-extrabold text-[11px] sm:text-xs text-center shadow-md flex items-center justify-center gap-1.5 hover:opacity-95 transition-all truncate"
        >
          <Moon className="w-3.5 h-3.5 text-yellow-200 fill-yellow-200 shrink-0" />
          <span className="truncate">Kuthi Yengba</span>
        </Link>

        <Link
          href="/manipuri_kuthi"
          className="flex-1 py-2.5 px-2 rounded-full bg-[#5c2406] text-white border border-[#fde68a]/60 font-extrabold text-[11px] sm:text-xs text-center shadow-md flex items-center justify-center gap-1.5 hover:opacity-95 transition-all truncate"
        >
          <Sparkles className="w-3.5 h-3.5 text-yellow-300 fill-yellow-300 shrink-0" />
          <span className="truncate">Kuthi Iba</span>
        </Link>
      </div>
